...
const CheckPage = () => {
    ...
    const handleAddMedication = () => {
        const selectedMedicine = medicines.find(med => med.medicine_id.toString() === selectedSubtype);
        if (!selectedMedicine) return;

        const dosageType = selectedPatient && selectedPatient.age < 12 ? 'child' : 'adult';
    ...
    const displayRemaining = (medicineId: string) => {
        const subtype = medicines.find(sub => sub.medicine_id.toString() === medicineId);
                                            ...
                                            {medicines
                                                ...
                                                .map(medicine => (
                                                    <SelectItem key={medicine.medicine_id} value={medicine.medicine_id.toString()}>
                                                        {medicine.medicine_name} (Stock: {medicine.stock_quantity})
                                                    </SelectItem>
                                                ))}
                                        </SelectContent>
                                    </Select>
